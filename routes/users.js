...
router.post('/create-user', upload.single('userImage'), async (req, res) => {
  try {
    const { firstName, lastName, email, phoneNum, password, address, city, dob, userImage } = req.body;
    ...
    const imageUploadResult = await cloudinary.uploader.upload(userImage, {
      folder: "user-images", // Optional: specify a folder in your Cloudinary account
    });
